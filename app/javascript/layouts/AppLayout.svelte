<script>
  ...
  const railVisitOptions = {}
  let pendingRailNavigation
  ...
  onMount(() => {
    ...
    return () => {
      ...
      pendingRailNavigation = undefined
  ...
  function startRailNavigation(event, href, fullPage = false) {
    ...
    cancelRailPrefetch(href)
    if (fullPage) window.location.assign(href)
    else visitRail(href)
  }

  function visitRail(href) {
    pendingRailNavigation = href
    const prefetch = router.getPrefetching(href, railVisitOptions)
    if (!prefetch?.inFlight) {
      pendingRailNavigation = undefined
      router.visit(href, railVisitOptions)
      return
    }

    const finish = () => {
      if (pendingRailNavigation !== href) return
      pendingRailNavigation = undefined
      router.visit(href, railVisitOptions)
    }
    prefetch.response.then(finish, finish)
  }
  ...
  function scheduleRailPrefetch(href, fullPage = false) {
    ...
    const timer = window.setTimeout(() => {
      ...
      if (fullPage) {
        ...
        router.prefetch(href, railVisitOptions, { cacheFor: "30s" })
      }
    }, 75)
